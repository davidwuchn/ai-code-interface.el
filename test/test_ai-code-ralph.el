    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'ai-code--get-files-directory)
            ...
            (should (string-match-p "CONTEXT BOUNDARY" sent-prompt))
            (should (string-match-p "Ignore prior task context" sent-prompt))
            (should (string-match-p "Task ID:" sent-prompt))
            (let ((content (ai-code-ralph-test--read-file task-file)))
              ...
              (should (string-match-p "TASK ID:" content))
