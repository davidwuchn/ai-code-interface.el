...
The package includes support for desktop notifications to alert you when AI responses are complete. This is especially useful when:
- Working with multiple AI sessions
- Switching between tasks while waiting for AI responses
- The AI session buffer is not currently visible

*** Enabling Notifications

Notifications are enabled by default. To configure them, add the following to your configuration:
...
#+begin_src emacs-lisp
  ...
  (setq ai-code-notifications-on-response-complete t)  ; Notify when AI completes a response
  (setq ai-code-notifications-on-error t)              ; Notify on errors
  (setq ai-code-notifications-on-session-end t)        ; Notify when session terminates
  
  ;; Only notify when the AI buffer is not in focus (default: t)
  (setq ai-code-notifications-only-when-not-focused t)
  
  ;; Set notification urgency level: 'low, 'normal, or 'critical
  (setq ai-code-notifications-urgency 'normal)
  
  ;; Set notification timeout in milliseconds (0 = no timeout, -1 = system default)
...
- GNU/Linux or other systems with D-Bus support
- The =notifications= package (included with Emacs on supported systems)

On systems without notification support, the package will fall back to displaying messages in the echo area.
